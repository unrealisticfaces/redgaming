...
export default function Home() {
  ...
  // Quadruple the list to ensure a seamless loop with absolutely zero gaps
  ...
  return (
    <div className="flex flex-col items-center justify-center py-12 w-full animate-in fade-in duration-1000">
      ...
      <div className="w-full max-w-[100vw] relative mb-32 z-10 overflow-hidden">
        ...
        {/* Shadow Overlays for smooth side-fading */}
        ...
        <div className="w-full relative h-[300px]">
          {loading ? (
            ...
            /* ENHANCED: transform-gpu and will-change-transform shift layout processing directly to the GPU */
            <div 
              className="flex animate-marquee gap-5 px-5 transform-gpu will-change-transform hover:[animation-play-state:paused] transition-all duration-300 ease-out"
              style={{ animationDuration: '60s' }}
            >
              {marqueeGames.map((game, index) => (
                <div key={`${game.id}-${index}`} className="w-[180px] sm:w-[220px] flex-shrink-0 bg-[#141414] rounded-sm border border-white/10 overflow-hidden transition-all duration-500 group relative shadow-xl transform-gpu backface-hidden">
                  ...
                  <div className="aspect-[3/4] w-full bg-[#111] relative flex items-center justify-center overflow-hidden">
                    {game.image ? (
                      <img 
                        ...
                        loading="lazy" 
                        decoding="async" 
                        className="absolute inset-0 w-full h-full object-cover z-0 opacity-80 group-hover:opacity-100 transition-opacity duration-500 transform-gpu" 
                      />
                    ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl px-6 lg:px-0 z-10">
        <div className="bg-[#141414]/80 backdrop-blur-xl border border-white/10 p-8 flex flex-col items-center hover:border-red-500/50 transition-all duration-500 group rounded-sm shadow-2xl hover:shadow-[0_0_30px_rgba(239,68,68,0.2)] hover:-translate-y-1 transform-gpu">
