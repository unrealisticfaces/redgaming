              ...
              <div className="flex items-center gap-3">
                ...
                <div className="relative">
                  <select 
                    ...
                    className="bg-[#222] border border-white/10 text-xs text-white font-bold py-1.5 pl-3 pr-8 rounded-sm focus:outline-none focus:border-red-500 cursor-pointer appearance-none"
                  >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-5">
          {filteredGames.map(game => {
            ...
            return (
              <div 
                key={game.id} 
                className={`w-full bg-[#141414] rounded-sm border overflow-hidden hover:-translate-y-1.5 transition-all duration-500 group flex flex-col relative shadow-lg ${
                  isSelected 
              ...
              >
                ...
                <div className={`aspect-[3/4] w-full bg-[#111] relative overflow-hidden flex items-center justify-center border-b ${isSelected ? 'border-red-500/40' : 'border-white/10'}`}>
                  {game.image ? (
                    <img src={game.image} alt={game.title} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover z-0 transition-transform duration-700 group-hover:scale-110" />
                  ) : (
                    <div className="absolute inset-0 bg-gradient-to-b from-[#222] to-[#141414] z-0"></div>
                  )}
                  <div className={`absolute inset-0 transition-colors duration-500 z-10 ${isSelected ? 'bg-black/20' : 'bg-black/10 group-hover:bg-black/60'}`}></div>
                  <div className="hidden md:flex absolute inset-0 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-20">
                    <button
                      ...
                      className={`px-4 py-2.5 rounded-sm font-black text-[10px] uppercase tracking-widest flex items-center gap-2 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 ${
                        isSelected
                ...
                <div className="p-4 flex flex-col flex-grow bg-gradient-to-t from-[#111] to-[#141414]">
                  ...
                  <div className="mt-auto flex flex-col gap-3">
                    ...
                    <button
                      onClick={() => toggleGameSelection(game)}
                      className={`flex md:hidden w-full py-2.5 rounded-sm font-black text-[10px] uppercase tracking-widest items-center justify-center gap-2 transition-all duration-300 ${
                        isSelected
